...
def get_kpoint_divisions_from_kspacing(
...
class CheckKpointsKspacing(BaseValidator):
        ...
        if (kspacing := vasp_files.valid_input_set.incar.get("KSPACING")) and (
            ...
            valid_num_kpts = np.prod(
                get_kpoint_divisions_from_kspacing(
                    vasp_files.user_input.structure, valid_kspacing
                ),
                dtype=int,
            )
    ...
    def _check_user_shifted_mesh(
        ...
        if (
            ...
            and any(
                shift_val != 0 for shift_val in vasp_files.actual_kpoints.kpts_shift
            )
        ):  # type: ignore[union-attr]
            reasons.append(
                "INPUT SETTINGS --> KPOINTS: shifting the kpoint mesh is not currently allowed."
            )

    def _check_explicit_mesh_permitted(
        self, vasp_files: VaspFiles, reasons: list[str], warnings: list[str]
    ) -> None:
            ...
            cur_num_kpts = np.prod(
                get_kpoint_divisions_from_kspacing(
                    ...
                    vasp_files.user_input.incar.get(
                        "KSPACING", self.vasp_defaults["KSPACING"].value
                    ),
                ),
                dtype=int,
            )
    ...
    def _check_kpoint_mesh_symmetry(
        self, vasp_files: VaspFiles, reasons: list[str], warnings: list[str]
    ) -> None:
        ...
        if vasp_files.actual_kpoints:
            ...
            cur_kpoint_style = (
                ...
                if vasp_files.user_input.incar.get(
                    "KGAMMA", self.vasp_defaults["KGAMMA"].value
                )
                else "monkhorst"
            )
        ...
        is_face_centered = (
            vasp_files.user_input.structure.get_space_group_info()[0][0] == "F"
        )
