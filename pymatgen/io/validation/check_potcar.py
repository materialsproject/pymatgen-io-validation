...
class CheckPotcar(BaseValidator):
    ...
    data_match_tol: float = Field(
        1.0e-6, description="Tolerance for matching POTCARs to summary statistics data."
    )
    ignore_header_keys: set[str] | None = Field(
        {"copyr", "sha256"},
        description="POTCAR summary statistics keywords.header fields to ignore during validation",
    )
    ...
    def auto_fail(
        ...
        if self.potcar_summary_stats_path is None:
            ...
            ps.keywords is None or ps.stats is None
            for ps in vasp_files.user_input.potcar
        ):
            reasons.append(
    ...
    def _check_potcar_spec(
        ...
        if vasp_files.valid_input_set.potcar:
            ...
            valid_potcar_summary_stats: dict[str, list[dict[str, Any]]] = {
                p.titel.replace(" ", ""): [p.model_dump()]
                for p in vasp_files.valid_input_set.potcar
            }
        elif vasp_files.valid_input_set._pmg_vis:
            # Fallback, use the stats from pymatgen - only load and cache summary stats here.
            psp_subset = self.potcar_summary_stats.get(
                vasp_files.valid_input_set.potcar_functional, {}
            )

            valid_potcar_summary_stats = {}
            for (
                element
            ) in vasp_files.user_input.structure.composition.remove_charges().as_dict():
                potcar_symbol = vasp_files.valid_input_set._pmg_vis._config_dict[
                    "POTCAR"
                ][element]
        ...
        try:
            ...
            for potcar in vasp_files.user_input.potcar:  # type: ignore[union-attr]
                reference_summary_stats = valid_potcar_summary_stats.get(
                    potcar.titel.replace(" ", ""), []
                )
            ...
            if len(incorrect_potcars) > 0:
                # format error string
                incorrect_potcars = [
                    potcar.split("_")[0] for potcar in incorrect_potcars
                ]
                if len(incorrect_potcars) == 1:
                    incorrect_potcar_str = incorrect_potcars[0]
                else:
                    incorrect_potcar_str = (
                        ", ".join(incorrect_potcars[:-1])
                        + f", and {incorrect_potcars[-1]}"
                    )  # type: ignore
