...
class VaspParam:
    ...
    Args:
        ...
        value (int, float, bool, list, None) : the default value of this parameter if
    ...
    __slots__ : tuple[str] = ("name", "value","operation", "alias", "tag", "tolerance", "comment",)
    
    def __init__(
        self,
        name : str,
        value : int | float | bool | list | None,
    ...
    def __dict__(self) -> dict[str,int | float | bool | None]:
...
VASP_DEFAULTS_LIST = [
    ...
    VaspParam("DEPER", 0.3, "misc", operation = "==", comment = "According to the VASP manual, DEPER should not be set by the user."),
    VaspParam("EBREAK", None, "post init", comment = "According to the VASP manual, EBREAK should not be set by the user."),
    ...
    VaspParam("NWRITE", 2, "write", operation = ">=", comment = "The specified value of NWRITE does not output all needed information."),
